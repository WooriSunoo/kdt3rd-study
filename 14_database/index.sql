...
-- 제약조건
-- NOT NULL: NULL 허용 x
-- AUTO_INCREMENT: 자동 숫자 증가(데이터들어올때마다 숫자값증가)
-- PRIMARY KEY: 기본키!
-- DEFAULT: 기본값 DEFAULT 'x'
-- UNIQUE: 중복 허용 x
CREATE TABLE user (
...
-- [CREATE - INSERT]
-- : 데이터 추가
-- 참고) AUTO_INCREMENT 컬럼의 값음 굳이 넣지 않으면 알아서 증가
-- NULL 이라고 넣으면 알아서 숫자가 지정되기도 함
-- 주의) AUTO_INCREMENT 컬럼은반드시 PK(기본키, primary key)여야 함
...
SELECt id,age FROM user WHERE name ='이지은'; -- 이름이 '이지은'인 사람의 id, age조회
...
SELECT * FROM user WHERE id > 6 ORDER BY age ASC; -- id 6초과인 사람 중에서 age컬럼 오름차순 정렬
...
-- 질문) 구문에서 이름이 4글자나 2글자인 사람은 제외되나?  네! 가가희가, 가희 일경우 안잡힘
...
-- 이름의 마지막 글자가 '희'인 사람들 나이기준으로 내림차순
...
-- 데이터 갱신(수정)
-- 참고) Error Code: 1175.
--테이블 데이터를 update/delete하려고 할 때 key칼럼을 이용해서 수정/삭제하는것이 바람직
...
-- [DELETE - DELETE]
-- : 데이터 삭제
-- 주의) delete where절 미사용시, 모든 향의 데이터가 삭제됨
